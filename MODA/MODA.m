...
load("../model_validate_res.mat")
load("../y_validate.mat")
y1=model_validate_res(:, 1);
y2=model_validate_res(:, 2);
y3=model_validate_res(:, 3);
y4=model_validate_res(:, 4);
...
for iter=1:max_iter
    ...
    for i=1:N %Calculate all the objective values first
        Particles_F(i,:)=ObjectiveFunction(X(:,i)', y1, y2, y3, y4, actual);
